...
class BytenodeWebpackPlugin implements WebpackPluginInstance {
  ...
  apply(compiler: Compiler): void {
    ...
    if (this.options.compileForElectron) {
      ...
      if (target) {
        ...
        if (!targets.some((target) => target.startsWith("electron-"))) {
          logger.warn(
            `Not using bytenode because [${targets.join(
              ", "
            )}] is marked as "compileForElectron: true" but has "target: 'web'".`
          );
          return;
        }
      }
    ...
    const {
      entries: { ignored, loaders, targets },
      modules,
    } = prepare(compiler);
    ...
    if (this.options.preventSourceMaps) {
      logger.log(
        "Preventing source maps from being generated by changing webpack.options.devtool to false."
      );
    ...
    if (this.options.compileForElectron) {
      ...
      if (target) {
        ...
        if (!targets.some((target) => target.startsWith("electron-"))) {
          logger.warn(
            `Consider using an electron target instead of or in addition to [${targets.join(
              ", "
            )}] when compiling for electron.`
          );
        }
      }
    }
    ...
    new ExternalsPlugin(
      "commonjs",
      ({ context, contextInfo, request }, callback) => {
    ...
    new VirtualModulesPlugin(Object.fromEntries(modules.entries())).apply(
      compiler
    );
    ...
    compiler.hooks.afterPlugins.tap(this.name, () => {
      ...
      compiler.hooks.compilation.tap(this.name, (compilation) => {
        ...
        compilation.hooks.processAssets.tapPromise(
          { name: this.name, stage: Compilation.PROCESS_ASSETS_STAGE_DERIVED },
          async (assets): Promise<void> => {
            ...
            for (const [name, asset] of Object.entries(assets)) {
              ...
              if (loaderOutputFiles.includes(name)) {
                await updateLoaderToRequireCompiledAssets(
                  compilation,
                  name,
                  asset
                );
              } else if (isTargetExtension(name) && matches(name)) {
                await updateTargetWithCompiledCode(
                  compilation,
                  name,
                  asset,
                  this.options
        ...
        async function updateLoaderToRequireCompiledAssets(
          ...
          const source = await replaceSource(asset, (raw) => {
            ...
            for (let index = 0; index < targets.size; index++) {
              ...
              logger.debug("replacer", {
                name,
                target: { name: Array.from(targets.keys())[index], ...target },
                fromLocation,
                toLocation,
              });
              ...
              for (const from of target.import) {
                raw = replaceImportPath(raw, name, from, to, {
                  permutations: [normalizeCodePath],
                });
              }
            }
            ...
            for (const file of targetOutputFiles) {
              raw = replaceImportPath(
                raw,
                name,
                file,
                fromTargetToCompiledExtension(file),
                {
                  permutations: [
                    normalizeCodePathForUnix,
                    normalizeCodePathForWindows,
          ...
          function replaceImportPath(
            ...
            from: string,
            to: string,
            options?: { permutations?: Array<(path: string) => string> }
          ): string {
            const { permutations = [(identity: string): string => identity] } =
              options ?? {};
    ...
    async function updateTargetWithCompiledCode(
      compilation: Compilation,
      name: string,
      asset: Source,
      options: Options
      ...
      if (options.keepSource) {
        logger.debug(
          "re-emitting decompiled asset due to plugin.options.keepSource being true"
        );
        compilation.emitAsset(name, asset);
      } else {
        logger.debug(
          "NOT re-emitting decompiled asset due to plugin.options.keepSource being false"
        );
      }
    }
  }
...
function collectOutputFiles(
  ...
  for (const name of from.keys()) {
    ...
    if (entrypoint) {
      files.push(
        ...entrypoint.chunks.flatMap((chunk) =>
...
function prepare(compiler: Compiler): Prepared {
  ...
  if (typeof entry === "function") {
    throw new Error(
      "webpack.options.entry cannot be a function, use strings or objects"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "webpack.options.output.filename cannot be static, use a dynamic one like [name].js"
    );
  }
  ...
  for (const [name, descriptor] of Object.entries(entry)) {
    if (descriptor.filename) {
      throw new Error(
        "webpack.options.entry.filename is not supported, use webpack.options.output.filename"
      );
    }
    ...
    entries.loaders.set(name, {
      import: imports.map((file) => toLoaderFileName(file)),
    });

    // generates virtual modules with the code of the loader files
    for (const file of imports) {
      const code = createLoaderCode({
        imports: [toSiblingRelativeFileLocation(file)],
      });
...
function setupLifecycleLogging(
  ...
  setupHooksLogging(
    name,
    "compiler",
    compiler.hooks as unknown as Record<string, Hook<any, any>>
  );
  ...
  compiler.hooks.compilation.tap(name, (compilation) => {
    setupHooksLogging(
      name,
      "compilation",
      compilation.hooks as unknown as Record<string, Hook<any, any>>
    );
  });
  ...
  compiler.hooks.normalModuleFactory.tap(name, (normalModuleFactory) => {
    setupHooksLogging(
      name,
      "normalModuleFactory",
      normalModuleFactory.hooks as unknown as Record<string, Hook<any, any>>
    );
  });
